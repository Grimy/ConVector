...
class Drawing implements Iterable<Drawing.Splash> {
	...
	void paint(final Color color, final Area area) {
		if (area.isEmpty()) {
			return;
		}
		bounds.unionWith(area.getBounds());
		splashes.add(new Splash(color, area));
	}
	...
	*/
	...
	void optimize() {
		...
		for (int i = 1; i < numSplashes; i++) {
			...
			for (int j = i; j < numSplashes; j++) {
				final double dist = getDistance(splashes.get(i - 1).area, splashes.get(j).area);
	...
	static class Splash {
		final Color color;
		final Area area;
		Splash(final Color color, final Area area) {
			this.color = color;
			this.area = area;
		}

		PathIterator getPathIterator(final BaseTransform transform, final int flatness) {
			return flatness < 0 ? area.getPathIterator(transform)
			                    : area.getPathIterator(transform, flatness);
		}
	}

	/* Returns the distance between the first points of two Shapes. */
	private double getDistance(final Area a, final Area b) {
